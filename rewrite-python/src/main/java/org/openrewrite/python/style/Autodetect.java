...
public class Autodetect extends NamedStyles implements PythonStyle {
    ...
    private static class FindSpaceVisitor extends PythonIsoVisitor<SpacesStatistics> {
        ...
        public Py.ForLoop visitForLoop(Py.ForLoop forLoop, SpacesStatistics statistics) {
            // Check space before semicolon (which is used in for loop conditions)
            statistics.otherBeforeColon += hasSpace(forLoop.getPadding().getIterable().getBefore());
            return super.visitForLoop(forLoop, statistics);
        }
