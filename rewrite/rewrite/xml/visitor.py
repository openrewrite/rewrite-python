...
class XmlVisitor(TreeVisitor[Xml, P]):
    ...
    def visit_document(self, document: Document, p: P) -> Xml:
        ...
        document = document.with_prolog(self.visit_and_cast(document.prolog, p, Prolog))
        document = document.with_root(self.visit_and_cast(document.root, p, Tag))
        return document

    def visit_prolog(self, prolog: Prolog, p: P) -> Xml:
        prolog = prolog.with_markers(self.visit_markers(prolog.markers, p))
        prolog = prolog.with_xml_decl(self.visit_and_cast(prolog.xml_decl, p, XmlDecl))
        prolog = prolog.with_misc([self.visit_and_cast(v, p, Misc) for v in prolog.misc])
        prolog = prolog.with_jsp_directives([self.visit_and_cast(v, p, JspDirective) for v in prolog.jsp_directives])
        return prolog

    def visit_xml_decl(self, xml_decl: XmlDecl, p: P) -> Xml:
        xml_decl = xml_decl.with_markers(self.visit_markers(xml_decl.markers, p))
        xml_decl = xml_decl.with_attributes([self.visit_and_cast(v, p, Attribute) for v in xml_decl.attributes])
        return xml_decl
    ...
    def visit_processing_instruction(self, processing_instruction: ProcessingInstruction, p: P) -> Xml:
        ...
        processing_instruction = processing_instruction.with_processing_instructions(self.visit_and_cast(processing_instruction.processing_instructions, p, CharData))
        return processing_instruction

    def visit_tag(self, tag: Tag, p: P) -> Xml:
        tag = tag.with_markers(self.visit_markers(tag.markers, p))
        tag = tag.with_attributes([self.visit_and_cast(v, p, Attribute) for v in tag.attributes])
        tag = tag.with_content([self.visit_and_cast(v, p, Content) for v in tag.content])
        tag = tag.with_closing(self.visit_and_cast(tag.closing, p, Tag.Closing))
        return tag
    ...
    def visit_attribute(self, attribute: Attribute, p: P) -> Xml:
        ...
        attribute = attribute.with_key(self.visit_and_cast(attribute.key, p, Ident))
        attribute = attribute.with_value(self.visit_and_cast(attribute.value, p, Attribute.Value))
        return attribute
    ...
    def visit_doc_type_decl(self, doc_type_decl: DocTypeDecl, p: P) -> Xml:
        ...
        doc_type_decl = doc_type_decl.with_name(self.visit_and_cast(doc_type_decl.name, p, Ident))
        doc_type_decl = doc_type_decl.with_external_id(self.visit_and_cast(doc_type_decl.external_id, p, Ident))
        doc_type_decl = doc_type_decl.with_internal_subset([self.visit_and_cast(v, p, Ident) for v in doc_type_decl.internal_subset])
        doc_type_decl = doc_type_decl.with_external_subsets(self.visit_and_cast(doc_type_decl.external_subsets, p, DocTypeDecl.ExternalSubsets))
    ...
    def visit_doc_type_decl_external_subsets(self, external_subsets: DocTypeDecl.ExternalSubsets, p: P) -> Xml:
        ...
        external_subsets = external_subsets.with_elements([self.visit_and_cast(v, p, Element) for v in external_subsets.elements])
    ...
    def visit_element(self, element: Element, p: P) -> Xml:
        ...
        element = element.with_subset([self.visit_and_cast(v, p, Ident) for v in element.subset])
        return element
    ...
    def visit_jsp_directive(self, jsp_directive: JspDirective, p: P) -> Xml:
        ...
        jsp_directive = jsp_directive.with_attributes([self.visit_and_cast(v, p, Attribute) for v in jsp_directive.attributes])
